...
GPUParticle::GPUParticle() : m_particles(nullptr), m_maxParticles(0),m_position(0,0,0),m_drawShader(0), m_updateShader(0) {
	m_vao[0] = 0;
	m_vao[1] = 0;
	m_vbo[0] = 0;
	m_vbo[1] = 0;
...
void GPUParticle::initalise(unsigned int a_maxParticles,  float a_lifetimeMin, float a_lifetimeMax,  float a_velocityMin, float a_velocityMax, float a_startSize, float a_endSize,  const glm::vec4& a_startColour, const glm::vec4& a_endColour) {
	...
	m_particles = new GPUParticleS[ a_maxParticles ];
...
void GPUParticle::createBuffers() {
	...
	glBufferData(GL_ARRAY_BUFFER, m_maxParticles * sizeof(GPUParticleS), m_particles, GL_STREAM_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, m_maxParticles * sizeof(GPUParticleS), 0, GL_STREAM_DRAW);
...
void GPUParticle::createDrawShader() {
	unsigned int vs = Utility::loadShader("shaders/06_GPUParticle.vert", GL_VERTEX_SHADER);
	unsigned int fs = Utility::loadShader("shaders/06_GPUParticle.frag", GL_FRAGMENT_SHADER);
	unsigned int gs = Utility::loadShader("shaders/06_GPUParticle.geom", GL_GEOMETRY_SHADER);
...
void GPUParticle::createUpdateShader() {
	...
	unsigned char* source = Utility::fileToBuffer("shaders/06_ParticleUpdate.vert");
...
void GPUParticle::draw(const glm::mat4& a_cameraTransform, const glm::mat4& a_projection,unsigned int TexID) {
	//////////////////////////////////////////////////////////////////////////
	...
	GLint uDiffuseTexture = glGetUniformLocation(m_drawShader, "DiffuseTexture");
